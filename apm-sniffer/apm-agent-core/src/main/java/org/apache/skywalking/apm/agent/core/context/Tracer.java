...
import static org.apache.skywalking.apm.agent.core.context.internal.Lists.concatImmutableLists;
...
import org.apache.skywalking.apm.agent.core.context.internal.recorder.PendingSpan;
...
public class Tracer {
     ...
     * @see Propagation
     *
     * @see org.apache.skywalking.apm.agent.core.context.propagation.TraceContext.Extractor#extract(Object)
    ...
    private TraceContext newRootContext() {
    ...
    private TraceContext nextContext(TraceContext parent) {
        return nextContext(InternalPropagation.instance.flags(parent),
            parent.traceIdHigh(),
            parent.traceId(),
            parent.localRootId(),
            parent.spanId(),
    ...
    private TraceContext nextContext(int flags, long traceIdHigh, long traceId, long localRootId,
        long parentId, List<Object> extra) {
        if (alwaysSampleLocal && (flags & FLAG_SAMPLED_LOCAL) != FLAG_SAMPLED_LOCAL) {
            flags |= FLAG_SAMPLED_LOCAL;
        }
        long nextId = nextId();
        ...
        // Zero when root or an externally managed context was passed to newChild or
        // scopedWithParent
        if (localRootId == 0L) {
            localRootId = spanId;
        }
    ...
    public Span nextSpan(TraceContextOrSamplingFlags extracted) {
        Assert.notNull(extracted, "extracted can not be null");
        ...
        if (traceIdContext != null) {
            return _toSpan(
                nextContext(InternalPropagation.instance.flags(extracted.traceIdContext()),
        ...
        if (implicitParent != null) {
            // At this point, we didn't extract trace IDs, but do have a trace in progress. Since
            // typical
    ...
    private TraceContext decorateExternal(TraceContext context) {
        if (context == null) {
        ...
        if (alwaysSampleLocal) {
            ...
            if ((flags & FLAG_SAMPLED_LOCAL) != FLAG_SAMPLED_LOCAL) {
                context = InternalPropagation.instance
                    .withFlags(context, flags | FLAG_SAMPLED_LOCAL);
            }
        }
    ...
    private Span _toSpan(TraceContext decorated) {
        if (isNoop(decorated)) {
            return new NoopSpan(decorated);
        }
        // allocate a mutable span in case multiple threads call this method.. they'll use the
        // same data
        PendingSpan pendingSpan = pendingSpans.getOrCreate(decorated, false);
        return new RealSpan(decorated, pendingSpans, pendingSpan.state(), clock,
            finishedSpanHandler);
    }
    ...
    private boolean isNoop(TraceContext context) {
