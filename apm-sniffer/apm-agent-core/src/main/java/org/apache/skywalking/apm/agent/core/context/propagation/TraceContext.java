...
import static org.apache.skywalking.apm.agent.core.context.internal.HexCodec.lenientLowerHexToUnsignedLong;
import static org.apache.skywalking.apm.agent.core.context.internal.HexCodec.toLowerHex;
import static org.apache.skywalking.apm.agent.core.context.internal.HexCodec.writeHexLong;
import static org.apache.skywalking.apm.agent.core.context.internal.InternalPropagation.FLAG_LOCAL_ROOT;
...
import static org.apache.skywalking.apm.agent.core.context.internal.InternalPropagation.FLAG_SHARED;
import static org.apache.skywalking.apm.agent.core.context.internal.Lists.ensureImmutable;
...
import org.apache.skywalking.apm.agent.core.context.internal.RecyclableBuffers;
...
public final class TraceContext extends SamplingFlags {
     ...
     *
     * @see org.apache.skywalking.apm.agent.core.context.Tracer#nextSpan(TraceContextOrSamplingFlags)
     */
    ...
    private volatile String traceIdString; // Lazily initialized and cached.
    ...
    private volatile String parentIdString; // Lazily initialized and cached.
    ...
    private volatile String localRootIdString; // Lazily initialized and cached.
    ...
    private volatile String spanIdString; // Lazily initialized and cached.
    ...
    public static final class Builder {
        ...
        final boolean parseTraceId(String traceIdString, Object key) {
            if (isNull(key, traceIdString)) {
                return false;
            }
            int length = traceIdString.length();
            if (invalidIdLength(key, length, 32)) {
                return false;
            }
        ...
        final <C, K> boolean parseParentId(Propagation.Getter<C, K> getter, C carrier, K key) {
            ...
            if (parentIdString == null) {
                return true; // absent parent is ok
            }
            int length = parentIdString.length();
            if (invalidIdLength(key, length, 16)) {
                return false;
            }
        ...
        boolean invalidIdLength(Object key, int length, int max) {
            ...
            Assert.state(max == 32 || max == 16, "max only be 32 or 16");
    ...
    private final long traceIdHigh, traceId, localRootId, parentId, spanId;

    private final List<Object> extra;

    TraceContext(int flags, long traceIdHigh, long traceId, long localRootId, long parentId,
    ...
    private volatile int hashCode; // Lazily initialized and cached.
