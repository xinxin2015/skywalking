...
import static org.apache.skywalking.apm.agent.core.context.internal.InternalPropagation.FLAG_DEBUG;
...
public final class B3SingleFormat {
  private static final int FORMAT_MAX_LENGTH = 32 + 1 + 16 + 3 + 16; // traceid128-spanid-1-parentid
  ...
  private static int writeB3SingleFormat(TraceContext context, long parentId, char[] result) {
    int pos = 0;
  ...
  private static long tryParseParentId(CharSequence b3, int pos, int endIndex) {
    if (endIndex < pos + 16) {
      Platform.get().log("Invalid input: truncated", null);
      return 0L;
    }
  ...
  private static TraceContextOrSamplingFlags tryParseSamplingFlags(CharSequence b3, int pos) {
  ...
  private static boolean checkHyphen(CharSequence b3, int pos) {
      if (b3.charAt(pos) == '-') {
          return true;
      }
  ...
  private static boolean notHexFollowsPos(CharSequence b3, int pos, int end) {
    return (end >= pos + 2) && !isLowerHex(b3.charAt(pos + 1));
  }

  private static long tryParse16HexCharacters(CharSequence lowerHex, int index, int end) {
  ...
  private static int parseFlags(CharSequence b3, int pos) {
    int flags;
  ...
  private static void logInvalidSampled(int pos) {
    Platform.get().log("Invalid input: expected 0, 1 or d for sampled at offset {0}", pos, null);
  }

  private static byte[] asciiToNewByteArray(char[] buffer, int length) {
